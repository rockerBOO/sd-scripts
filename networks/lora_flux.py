...
from library.network_utils import initialize_lora, initialize_parse_opts, initialize_pissa, initialize_urae
...
class LoRANetwork(torch.nn.Module):
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        if self.initialize is not None and (self.initialize[:5] == "pissa" or self.initialize[:4] == "urae"):
            ...
            with torch.no_grad():
                progress = tqdm(total=len(loras), desc="Converting weights")
                for lora in loras:
