# Minimum Inference Code for FLUX with Batch Support
...
def generate_images_batch(
    ...
    for batch_idx in range(0, total_images, batch_size):
        ...
        # Create noise for each seed in the batch
        noises = []
        ...
        # Concatenate all noises into a single batch
        batch_noise = torch.cat(noises, dim=0)
        
        # Prepare img_ids for the batch
        img_ids = flux_utils.prepare_img_ids(current_batch_size, packed_latent_height, packed_latent_width)
        
        # Prepare fp8 models (only on first batch)
        if batch_idx == 0:
        ...
        # Prepare embeddings (only encode once per unique prompt)
        if batch_idx == 0:
...
if __name__ == "__main__":
    ...
    if not args.interactive:
        ...
        if args.batch_size > 1 or args.n_batches > 1:
            total_images = args.batch_size * args.n_batches
            
            # Generate seeds for all images
            if args.start_seed is not None:
                seeds = [args.start_seed + i for i in range(total_images)]
            elif args.seed is not None:
                seeds = [args.seed + i for i in range(total_images)]
            else:
                base_seed = random.randint(0, 2**32 - 1)
                seeds = [base_seed + i for i in range(total_images)]
            
            logger.info(f"Batch generation: {args.n_batches} batches of {args.batch_size} images each")
            logger.info(f"Seeds: {seeds[0]} to {seeds[-1]}")
        ...
        while True:
            ...
            for opt in options[1:]:
                try:
                    ...
                    elif opt.startswith("s") and not opt.startswith("ss"):
                        steps = int(opt[1:].strip())
                    elif opt.startswith("ss"):
                        start_seed = int(opt[2:].strip())
                    ...
                    elif opt.startswith("n") and not opt.startswith("nb"):
            ...
            # Generate in batch or single mode
            if batch_size > 1 or n_batches > 1:
                total_images = batch_size * n_batches
                
                if start_seed is not None:
                    seeds = [start_seed + i for i in range(total_images)]
                ...
                generate_images_batch(
                    model, clip_l, t5xxl, ae, prompt, seeds, width, height, steps, guidance, negative_prompt, cfg_scale, batch_size
                )
            else:
