...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    # ) -> tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.Tensor, torch.Tensor | None, torch.Tensor | None, torch.Tensor]:
        ...
        def call_dit(img, img_ids, t5_out, txt_ids, l_pooled, timesteps, guidance_vec, t5_attn_mask, proportional_attention, ntk_factor, mod_vectors):
    ...
    def prepare_unet_with_accelerator(
        ...
        if not self.is_swapping_blocks:
            return super().prepare_unet_with_accelerator(args, accelerator, unet, optimizer)

        # if we doesn't swap blocks, we can move the model to device
        flux: flux_models.Flux = unet
        flux, optimizer = accelerator.prepare(flux, network, optimizer, device_placement=[not self.is_swapping_blocks])
