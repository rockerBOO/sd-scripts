...
from library.custom_train_functions import calculate_diffusion_dpo_loss
...
def test_diffusion_dpo_loss_basic():
    ...
    result, metrics = calculate_diffusion_dpo_loss(loss.mean([1, 2, 3]), ref_loss.mean([1, 2, 3]), beta_dpo)
...
def test_diffusion_dpo_loss_different_shapes():
    ...
    for shape in shapes:
        ...
        result, metrics = calculate_diffusion_dpo_loss(loss.mean([1, 2, 3]), ref_loss.mean([1, 2, 3]), 0.1)
...
def test_diffusion_dpo_loss_beta_values():
    ...
    for beta in beta_values:
        result, _ = calculate_diffusion_dpo_loss(loss, ref_loss, beta)
...
def test_diffusion_dpo_loss_implicit_acc():
    ...
    _, metrics = calculate_diffusion_dpo_loss(loss.mean((1, 2, 3)), ref_loss.mean((1, 2, 3)), 1.0)
...
def test_diffusion_dpo_gradient_flow():
    ...
    # Compute loss
    result, _ = calculate_diffusion_dpo_loss(loss, ref_loss, 0.1)
...
def test_diffusion_dpo_loss_chunking():
    ...
    result, metrics = calculate_diffusion_dpo_loss(loss.mean((1, 2, 3)), ref_loss.mean((1, 2, 3)), 1.0)
