...
def lora_dropout_down(down: Tensor, x: Tensor, dropout_prob=0.5):
    ...
    mask = torch.bernoulli(torch.ones(down.shape[1], device=down.device) * (1 - dropout_prob))
...
def lora_dropout_up(up: Tensor, x: Tensor, dropout_prob=0.5):
    ...
    mask = torch.bernoulli(torch.ones(up.shape[0], device=up.device) * (1 - dropout_prob))
...
def maybe_pruned_save(model, optimizer, enable_pruning=False, pruning_ratio=0.1):
    ...
    should_prune = enable_pruning and (hasattr(optimizer, "sampled_params"))
